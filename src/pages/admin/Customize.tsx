...
const Customize = () => {
  ...
  const saveContentChanges = () => {
    ...
    // Trigger iframe refresh to show updated content
  ...
  const handleThemeSelect = (index: number) => {
    ...
    updateColors({
      primary: colorThemes[index].primary,
      secondary: colorThemes[index].secondary,
    });
  };
  
  const handlePrimaryColorChange = (color: string) => {
    updateColors({ ...colors, primary: color });
  };
  
  const handleSecondaryColorChange = (color: string) => {
    updateColors({ ...colors, secondary: color });
  };
  
  const handleApplyTheme = () => {
    applyTheme();
    // Trigger iframe refresh to show updated theme
    setPreviewKey(prev => prev + 1);
  };
          ...
          <div className="lg:col-span-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  See how your changes will look on the live website
                </CardDescription>
              </CardHeader>
              <CardContent>
                <SitePreview key={`preview-${previewKey}`} />
              </CardContent>
            </Card>
          </div>
        </div>
